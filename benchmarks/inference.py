...
from olmo_core.nn.transformer import HydraTransformer, HydraTransformerConfig, TransformerConfig

VOCAB_SIZE = 100352


def build_hydra_model(n_heads=5, heads_depth=3, dtype=torch.bfloat16, device="cuda"):
    config = HydraTransformerConfig.from_olmo2_1B(n_heads=n_heads, heads_depth=heads_depth)
...
def benchmark_decode(model, prompt_ids, gen_length=128, step_interval=8, warmup_ms=100.0, rep_ms=1000.0):
    from benchmarks.harness import benchmark, compute_stats, filter_outliers_iqr
...
def make_output_dir():
    ...
    run_nums = [int(re.search(r"run(\d+)", name).group(1))
                for name in existing if re.search(r"run(\d+)", name)]
...
def run_benchmarks(model, prompt_ids, cfg, label):
    ...
    decode = benchmark_decode(
        model, prompt_ids, cfg["generation_length"],
        cfg["decode_step_interval"], cfg["warmup_ms"], cfg["rep_ms"],
    )
...
def main():
    ...
    if cfg["baseline"]:
        ...
        with torch.no_grad():
            results["baseline"] = run_benchmarks(baseline, prompt_ids, cfg, "Baseline (16-layer)")

    metadata = {
        "timestamp": datetime.now().isoformat(),
        "gpu": torch.cuda.get_device_name(0),
        **{k: cfg[k] for k in ["dtype", "n_heads", "heads_depth", "prompt_length",
                                "generation_length", "decode_step_interval"]},
    }
