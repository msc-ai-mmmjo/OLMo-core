...
def preprocess_example(example: dict[str, str], tokenizer: AutoTokenizer, max_seq_len: int) -> dict:
    ...
    first_chat = tokenizer.apply_chat_template(
        [{"role": "user", "content": first_prompt}], tokenize=False, add_generation_prompt=True
    )
    first_enc = tokenizer(first_chat, padding="max_length", truncation=True, max_length=max_seq_len)
