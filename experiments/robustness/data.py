...
def preprocess_example(
    ...
    assert gcg.num_return_seq == 1, "Please only use AmpleGCG.num_return_seq = 1 for now"
    ...
    clean_prompt = tokenizer.apply_chat_template(clean, tokenize=False, add_generation_prompt=True)
...
def load_shard(config: TrainingConfig, gcg: AmpleGCG) -> tuple[DataLoader, int, int]:
    ...
    shard_ds = shard_ds.map(
        preprocess_example,
        fn_kwargs={"tokenizer": tokenizer, "gcg": gcg, "max_seq_len": config.max_seq_len},
